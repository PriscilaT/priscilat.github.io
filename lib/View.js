...
var lang_EN = {
        "adequateTitle": "Adequate Learning Level",
        "correctAnswers": "Answer Questions Correctly",
        "disciplines": "Disciplines",
        "inadequateTitle": "Inadequate Learning Level",
        "login": "Number of Logins",
        "mockingTest": "Mocking Tests",
        "nivel": "Level",
        "points": "Earn Points",
        "questions": "Solve Questions",
        "trophies": "Unlock Trophies",
        "videos": "Watch Videos",
        "wrongAnswers": "Wrong Answers",
        "": "Empty",
        "tooltip-weigths": "Bigger is better: Interactions that are heavier for a student will help him in improving his performance."
};

var lang_BR = {
        "adequateTitle": "Nível de aprendizado adequado",
        "correctAnswers": "Responder Questões Corretamente",
        "disciplines": "Disciplines",
        "inadequateTitle": "Nível de aprendizado inadequado",
        "login": "Logins",
        "mockingTest": "Simulados",
        "nivel": "Nível",
        "points": "Ganhar Pontos",
        "questions": "Resolver Questões",
        "trophies": "Desbloquear Troféus",
        "videos": "Assistir Vídeos",
        "wrongAnswers": "Respostas Erradas",
        "": "Vazio",
        "tooltip-weigths": "Bigger is better: Interactions that are heavier for a student will help him in improving his performance."
};

exports.lang = lang_BR;
...
var histogramSize = {
        'correctAnswers': { 'width': 1, height: 2 },
        'disciplines': { 'width': 1, height: 1 },
        'login': { 'width': 1, height: 1 },
        'mockingTest': { 'width': 1, height: 1 },
        'nivel': { 'width': 1, height: 1 },
        'points': { 'width': 1, height: 1 },
        'questions': { 'width': 1, height: 2 },
        'trophies': { 'width': 1, height: 1 },
        'videos': { 'width': 1, height: 1 },
        'wrongAnswers': { 'width': 1, height: 2 },
        '': { 'width': 1, height: 1 }
};

exports.histogramSize = histogramSize;
...
exports.langHistogram = undefined;

var _lang = __webpack_require__(91);

var _lang2 = _interopRequireDefault(_lang);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var langHistogram_EN = {
        "adequateTitle": "Adequate Learning Level",
        "correctAnswers": "Answer Questions Correctly",
        "disciplines": "Disciplines",
        "inadequateTitle": "Inadequate Learning Level",
...
exports.langHistogram = langHistogram_BR;
...
var symbols = {
        "correctAnswers": "img/correct_answers.svg",
        "disciplines": "img/disciplines.svg",
        "login": "img/login.svg",
        "mockingTest": "img/mocking_test.svg",
        "nivel": "img/nivel.svg",
        "points": "img/points.svg",
        "questions": "img/questions.svg",
        "trophies": "img/trophies.svg",
        "videos": "img/videos.svg",
        "wrongAnswers": "img/wrong_answers.svg",
        "": "img/empty.svg",
        "-1": "img/low_interaction.svg",
        0: "img/medium_interaction.svg",
        1: "img/high_interaction.svg"
};
...
var tooltipDesc = {
    "correctAnswers": "Quantity of correctly answered questions of each Learning Group.",
    "disciplines": "Average number of disciplines finished by the students for each Learning Group",
    "inadequateTitle": "Inadequate Learning Level",
    "login": "Average Number of Sessions* for each Learning Group",
    "mockingTest": "Average number of answered mocking tests for each Learning Group.",
    "nivel": "Average students level for each Learning Group.",
    "points": "Average Earned Points for each Learning Group.",
    "questions": "Average solved questions for each Learning Group.",
    "trophies": "Average trophies unlocked for each Learning Group.",
    "videos": "Average number of Watched Videos for each Learning Group.",
    "wrongAnswers": "Average Number of wrong answers for each Learning Group. "
};

var tooltipDescGeneral = {
    "correctAnswers": "Quantity of correctly answered questions of each Learning Group.",
    "disciplines": "Average number of disciplines finished by the students for each Learning Group",
    "inadequateTitle": "Inadequate Learning Level",
    "login": "Average Number of Sessions* for each Learning Group",
...
exports.tooltipDesc = tooltipDesc;
...
var d3 = _interopRequireWildcard(_index);

var _histogram = __webpack_require__(469);

var _histogram2 = _interopRequireDefault(_histogram);

var _histograms = __webpack_require__(470);
...
console.log("O bom senso é a coisa do mundo mais bem distribuída: todos pensamos tê-lo em tal medida que até os mais difíceis de contentar nas outras coisas não costumam desejar mais bom senso do que aquele que têm.");
//import SpiderWebGraph from './spiderWeb.js';

//import HistogramSet from './histogramSet.js'
//import Weights from './weights.js';
//import Bubbles from './bubbles.js';
//import histogramSize from './dict/histogramSize.js';

module.exports = {
    Histograms: _histograms2.default,
...
var _histogramSize = __webpack_require__(173);

var _lang = __webpack_require__(91);

var _langHistogram = __webpack_require__(174);

var _symbols = __webpack_require__(175);

var _tooltipDesc = __webpack_require__(176);
...
var Histogram = function () {
  function Histogram(elementId) {
    _classCallCheck(this, Histogram);

    this.data = [{ key: "questions", value: { inadequate: 0, insufficient: 0, adequate: 0 } }];
    this.containerId = elementId;
    //this.barColor = "#426CE1";
  }
  ...
  _createClass(Histogram, [{
    ...
    value: function update() {
      console.log('update');
      //this.chartsContainer.remove().exit();
      this.container.remove().exit();
      this.plot(this.data);
    }
  }, {
    key: 'plotFromJson',
    value: function plotFromJson(path) {
      var self = this;
      var dataAux = void 0;
      d3.json(path, function (error, dataAux) {
    ...
    value: function plot(data) {
      ...
      height = 250; // - margin.top - margin.bottom - margin.caption;

      var y = d3.scaleLinear().domain([0, d3.max(this.data, function (d) {
        return d.value;
      })]).range([height, 0 + margin.top]);

      var x = d3.scaleBand().domain(this.data.map(function (d) {
        return d.key;
      })).range([0, width]).padding(.2).round(.01);
      ...
      var fill = d3.scaleLinear().domain([0, d3.max(this.data, function (d) {
        return d.value;
      })]).range([0, 4]);

      this.chartContainer = d3.select("#" + this.containerId).append("div").classed("chartContainer", true);

      this.svgChart = this.chartContainer.append('svg').classed('svgChart', true)
      // .style("width", width)
      // .style("height", height);
      ...
      this.chartBars = this.svgChart.selectAll("bars").data(this.data).enter();

      this.chartBars.append("rect").attr("class", function (d) {
        return d.key;
      }).classed("bars", true).attr("fill", function (d) {
        console.log(fill(d.value));
        return fill(d.value) >= 1 ? fill(d.value) >= 2 ? '#27ae60' : '#f1c40f' : '#c0392b';
      }).attr("y", function (d) {
        return y(d.value);
      }).attr("x", function (d) {
        return x(d.key);
      ...
      // Add the x Axis
      this.svgChart.append("g").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(x));

      //Valores do grafico
      this.chartBars.append("text").classed("label", true).attr("x", function (d) {
  ...
  _createClass(Histograms, [{
    ...
    value: function plotFromJson(path) {
      ...
      d3.json(path, function (error, dataAux) {
        ...
        dataAux.forEach(function (d) {
          d.key = d.key;
          d.value.inadequate = +d.value.inadequate;
          d.value.insufficient = +d.value.insufficient;
          d.value.adequate = +d.value.adequate;
        });
        self.plot(dataAux);
      });
    }
    ...
    value: function plot(data) {
      ...
      var width = 200,
          ...
          bar_width = 40,
          bar = { width: 40, gap: 10 },
          margin = { top: 10, right: 10, bottom: 20, left: 10, caption: 10 },
          sizeWidth = function sizeWidth(chartScale) {
        return height * chartScale;
      },
          posTextY = function posTextY(chartScale, marginTop, marginBottom) {
        return chartScale * height + marginTop + marginBottom;
      },
          linearScale = d3.scaleLinear().domain([0, d3.max(this.data, function (d) {
        return Math.max(d.value.adequate, d.value.inadequate, d.value.insufficient);
      })]).range([0, height - margin.top]);
      bar.gap = (width - bar.width * 3) / 3;

      this.container = d3.select("#" + this.containerId).append("div").classed("chartsContainer", true).style("grid-template-columns", "repeat(auto-fit, " + (width + margin.left + margin.right) + "px)").style("grid-template-rows", "minmax(50px, auto").style("justify-self", "center");

      this.chartsContainer = this.container.selectAll('.chart').data(this.data).enter().append('div').attr("class", function (d) {
        return d.key;
      }).classed("chart", true).style("grid-row", function (d) {
        return "span " + _histogramSize.histogramSize[d.key].height + "";
      }).style("grid-column", function (d) {
        return "span " + _histogramSize.histogramSize[d.key].width + "";
      });

      this.chartsContainer.append('div').classed('title', true).append('text').text(function (d) {
        return _langHistogram.langHistogram[d.key];
      });

      this.svgCharts = this.chartsContainer.append('svg').classed('svgChart', true).attr("width", width).attr("height", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom;
      });

      this.svgCharts.append("line").classed("x-axis", true).attr("x1", 0).attr("y1", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom - margin.caption;
      }).attr("x2", width).attr("y2", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom - margin.caption;
      });

      this.svgCharts.append("rect").classed("inadequate", true).attr("x", bar.width * 0 + bar.gap * 0.5).attr("y", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom - (margin.caption + linearScale(d.value.inadequate) * _histogramSize.histogramSize[d.key].height);
      }).attr("width", bar.width).attr("height", function (d) {
        return linearScale(+d.value.inadequate) * _histogramSize.histogramSize[d.key].height;
      });

      this.svgCharts.append("rect").classed("insufficient", true).attr("x", bar.width * 1 + bar.gap * 1.5).attr("y", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom - (margin.caption + linearScale(d.value.insufficient) * _histogramSize.histogramSize[d.key].height);
      }).attr("width", bar.width).attr("height", function (d) {
        return linearScale(+d.value.insufficient) * _histogramSize.histogramSize[d.key].height;
      });

      this.svgCharts.append("rect").classed("adequate", true).attr("x", bar.width * 2 + bar.gap * 2.5).attr("y", function (d) {
        return _histogramSize.histogramSize[d.key].height * height + margin.top + margin.bottom - (margin.caption + linearScale(d.value.adequate) * _histogramSize.histogramSize[d.key].height);
      }).attr("width", bar.width).attr("height", function (d) {
        return linearScale(d.value.adequate) * _histogramSize.histogramSize[d.key].height;
      });

      //Retorna a posição do texto dos gráficos. @chartScale: Tamanho da grade do gráfico, @chartValue: Valor do rect (puro)
      var sizeSVGy = function sizeSVGy(chartScale, chartValue) {
        return chartScale * height + margin.top + margin.bottom - (margin.caption + linearScale(chartValue) * chartScale + 2);
      };

      this.svgCharts.append("text").classed("label", true).attr("x", bar.width * 0 + bar.gap * 0.5 + bar.width * 0.5).attr("y", function (d) {
        return sizeSVGy(_histogramSize.histogramSize[d.key].height, d.value.inadequate);
      }).text(function (d) {
        return d.value.inadequate;
      });
      this.svgCharts.append("text").classed("label", true).attr("x", bar.width * 1 + bar.gap * 1.5 + bar.width * 0.5).attr("y", function (d) {
        return sizeSVGy(_histogramSize.histogramSize[d.key].height, d.value.insufficient);
      }).text(function (d) {
        return d.value.insufficient;
      });
      this.svgCharts.append("text").classed("label", true).attr("x", bar.width * 2 + bar.gap * 2.5 + bar.width * 0.5).attr("y", function (d) {
        return sizeSVGy(_histogramSize.histogramSize[d.key].height, d.value.adequate);
      }).text(function (d) {
        return d.value.adequate;
      });

      this.svgCharts.append("text").classed("caption", true).attr("x", bar.gap * 0.5 + bar.width * 0.5).attr("y", function (d) {
        return posTextY(_histogramSize.histogramSize[d.key].height, margin.top, margin.bottom);
      }).text("INADEQUATE");

      this.svgCharts.append("text").classed("caption", true).attr("x", bar.width * 1.5 + bar.gap * 1.5).attr("y", function (d) {
        return posTextY(_histogramSize.histogramSize[d.key].height, margin.top, margin.bottom);
      }).text("INSUFFICIENT");

      this.svgCharts.append("text").classed("caption", true).attr("x", bar.width * 2.5 + bar.gap * 2.5).attr("y", function (d) {
        return posTextY(_histogramSize.histogramSize[d.key].height, margin.top, margin.bottom);
      }).text("ADEQUATE");
      ...
      var tooltipChart = {
        ...
        width: 70
          ...
          tooltiplinearScale = d3.scaleLinear().domain([0, d3.max(this.data, function (d) {
        return Math.max(d.value.adequate, d.value.inadequate, d.value.insufficient);
      })]).range([0, tooltipChart.height]);

      this.tooltipContainer = this.chartsContainer.append("span").attr("class", "tooltipContainer").style("display", "none").style("opacity", 0).style("position", "absolute").style("width", width + margin.left + margin.right + "px");

      var tooltipGrid = this.tooltipContainer.append("div").classed("tooltipGrid", true).style("display", "inline").style("padding", "20px");

      var tooltipInfo = tooltipGrid.append("div").classed("tooltipInfo", true).style("margin", "16px").append("text").text(function (d) {
        return _tooltipDesc.tooltipDesc[d.key];
      });

      this.chartsContainer.on("mouseover", function () {
        d3.select(this).select(".tooltipContainer").transition().duration(100).style("opacity", 1).style("display", "inline");
      }).on("mouseout", function () {
        d3.select(this).select(".tooltipContainer").transition().duration(500).style("opacity", 0).style("display", "none");
      });
    }
  }]);
